...
export default function OverviewView({ 
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      ...
      <div className="border-b border-slate-800 pb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-white tracking-tight">
            {overviewData?.site_name || 'Plant A'}
          </h1>
      ...
      {cross_asset_alerts && cross_asset_alerts.length > 0 && (
        <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 flex items-start justify-between gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-slate-200">
                  Cross-Machine Pattern Detected: {cross_asset_alerts[0].pattern_title}
                </span>
                <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-slate-800 text-brand-400 border border-slate-700">
          ...
          <button
            ...
            className="text-xs font-semibold text-brand-400 hover:text-brand-300 flex items-center gap-1 whitespace-nowrap pt-0.5 transition-colors"
          >
                ...
                {isExpanded && (
                  <div className="px-5 pb-5 pt-1 border-t border-slate-800/80 space-y-4 text-xs animate-in fade-in duration-150">
                    ...
                    <div className="flex items-center justify-between pt-1">
                      ...
                      >
                        <span>Open Machine ({item.assetTag})</span>
                        <ArrowRight className="w-3.5 h-3.5" />
                      </button>
                    </div>
                  </div>
      ...
      {/* 4. MACHINE HEALTH — COMPACT OPERATIONAL ROWS */}
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="text-xs text-brand-400 hover:text-brand-300 font-medium flex items-center gap-1 transition-colors"
          >
            <span>All Machines ({assetsHealthList.length})</span>
            <ArrowRight className="w-3 h-3" />
          </button>
        </div>
          ...
          {assetsHealthList.map((mach) => (
            ...
            >
              ...
              <div className="flex items-center gap-2 self-end sm:self-center shrink-0">
                ...
                >
                  <span>{mach.status === 'Attention' ? 'Review' : 'Open Machine'}</span>
                  <ArrowRight className="w-3 h-3" />
                </button>
              </div>
            </div>
